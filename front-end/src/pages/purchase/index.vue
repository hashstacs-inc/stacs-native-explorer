    ...
    <article>
      <el-card class="p-search-box">
        <div class="p-search-content">
          ...
          <span>Name</span>
          <div class="p-search-input1">
            <el-input placeholder="Please enter the token ticker" v-model="queryData.symbol"></el-input>
          </div>
          <span></span>
          <div class="p-search-input1">
            <mu-button class="primary-button" style="background-color: #183A78;color: white" @click="getList">Search</mu-button>
          </div>
        </div>
      </el-card>
    </article>
...
<script>
  import {querySubscribesByPage} from '@/api'
  ...
  export default {
    data() {
      return {
        ...
        value: '',
        tableDesc: [
          {label: 'TX ID', prop: 'txId', width: 227, isA: true,target: 'purchaseDetail'},
          {label: 'Time Stamp +UTC', prop: 'createTime', width: '200'},
          {label: 'Address', prop: 'address', width: '', isA: true ,target: 'addressDetail'},
          {label: 'Amount', prop: 'amount', width: ''},
          {label: 'Token Name', prop: 'symbol', width: '', isA: true,  target: 'purchaseDetail',anchor: 'securityInfo'},
          {label: 'Paying Currency', prop: 'currencySymbol', width: '140'},
    ...
    methods: {
      ...
      getList() {
        ...
        querySubscribesByPage(this.queryData).then(res => {
      ...
      toDetail(row,target,anchor) {
        if (anchor === undefined || anchor === null || anchor === '') {
          this.$router.push({path:'/'+ target,query: { id:row.txId, amount:row.amount, type:'SUBSCRIBE', address:row.address,tokenIssue: 'Purchase',height:row.blockHeight }})
        } else {
          this.$router.push({path:'/'+ target,query: { id:row.txId, amount:row.amount, type:'SUBSCRIBE', address:row.address,tokenIssue: 'Purchase',height:row.blockHeight,anchor: anchor }})
        }
      }
    },
    mounted() {
...
<style lang="scss">
  ...
  .p-search-content {
    display: grid;
    grid-template-columns: 73px 300px 73px 300px 73px 300px;
